...
function library:saveconfig(folder_name, config_name)
	...
	if success then self:Notification({Title="Config Saved", Description="Saved: "..config_name, Time=3}) 
...
function library:loadconfig(folder_name, config_name)
	...
	if success then self:Notification({Title="Config Loaded", Description="Loaded: "..config_name, Time=3}) 
	else self:Notification({Title="Error", Description="Load failed: "..tostring(err)}) end
end

function library:settheme(theme,themeColor)
	local window = self
	if window.theme[theme] then window.theme[theme] = themeColor end
	if window.themeitems[theme] then
		for i,v in pairs(window.themeitems[theme]) do
			for z,x in pairs(v) do x[i] = themeColor end
		end
	end
end

function library:setkey(key)
	if typeof(key) == "EnumItem" then self.key = key end
end

function library:settoggle(side,bool)
	if side == "x" then self.x = bool else self.y = bool end
end

function library:setfont(newFont)
	if newFont ~= nil then
		local window = self
		for i,v in pairs(window.labels) do if v ~= nil then v.Font = newFont end end
	end
end
...
function library:settextsize(size)
	if size ~= nil then
		local window = self
		for i,v in pairs(window.labels) do if v ~= nil then v.TextSize = size end end
	end
end

-- // ADDED BACK THE MISSING PAGE FUNCTION
function library:page(props)
	local pageName = props.name or props.Name or "new page"
	local page = {}
	...
	local pageOutline = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(24, 24, 24),BorderColor3 = Color3.fromRGB(56, 56, 56),BorderMode = "Inset",BorderSizePixel = 1,Size = UDim2.new(1,0,1,0),Position = UDim2.new(0,0,0,0),Parent = tabbutton})
	...
	local r_line = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(56, 56, 56),BorderSizePixel = 0,Size = UDim2.new(0,1,0,1),Position = UDim2.new(1,0,1,1),ZIndex = 2,Parent = pageOutline})
	local l_line = utility.new("Frame",{AnchorPoint = Vector2.new(1,0),BackgroundColor3 = Color3.fromRGB(56, 56, 56),BorderSizePixel = 0,Size = UDim2.new(0,1,0,1),Position = UDim2.new(0,0,1,1),ZIndex = 2,Parent = pageOutline})
	local line = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(24, 24, 24),BorderSizePixel = 0,Size = UDim2.new(1,0,0,2),Position = UDim2.new(0,0,1,0),ZIndex = 2,Parent = pageOutline})
	local label = utility.new("TextLabel",{BackgroundTransparency = 1,Size = UDim2.new(1,0,0,20),Position = UDim2.new(0,0,0,0),Font = self.font,Text = pageName,TextColor3 = Color3.fromRGB(255,255,255),TextSize = self.textsize,TextStrokeTransparency = 0,Parent = pageOutline})
	...
	page = {["library"] = self,["outline"] = pageOutline,["r_line"] = r_line,["l_line"] = l_line,["line"] = line,["page"] = pageholder,["left"] = left,["right"] = right,["open"] = false,["pointers"] = {}}
	...
	if props.pointer then self.pointers[tostring(props.pointer)] = page.pointers end
...
function library:Notification(props)
	...
	local notifOutline = utility.new("Frame", {
		...
		BorderSizePixel = 0, Position = UDim2.new(1, 10, 1, -20 - offset + 50),
...
function library:loader(props)
	local loaderName = props.name or "Loader"
	local scriptname = props.scriptname or "Universal"
	local closed = props.close or function()end
	local logedin = props.login or function()end
	local loader = {}
	local loaderScreen = utility.new("ScreenGui",{Name = tostring(math.random(0,999999))..tostring(math.random(0,999999)),DisplayOrder = 9999,ResetOnSpawn = false,ZIndexBehavior = "Global",Parent = cre})
	if (check_exploit == "Synapse" and syn.request) then syn.protect_gui(loaderScreen) end
	local loaderOutline = utility.new("Frame",{AnchorPoint = Vector2.new(0.5,0.5),BackgroundColor3 = Color3.fromRGB(168, 52, 235),BorderColor3 = Color3.fromRGB(12, 12, 12),BorderSizePixel = 1,Size = UDim2.new(0,300,0,90),Position = UDim2.new(0.5,0,0.5,0),ZIndex = 9900,Visible = false,Parent = loaderScreen})
	local loaderOutline2 = utility.new("Frame",{AnchorPoint = Vector2.new(0.5,0.5),BackgroundColor3 = Color3.fromRGB(0, 0, 0),BorderColor3 = Color3.fromRGB(12, 12, 12),BorderSizePixel = 1,Size = UDim2.new(1,-4,1,-4),Position = UDim2.new(0.5,0,0.5,0),ZIndex = 9901,Parent = loaderOutline})
	local loaderIndent = utility.new("Frame",{AnchorPoint = Vector2.new(0.5,0.5),BackgroundColor3 = Color3.fromRGB(20, 20, 20),BorderColor3 = Color3.fromRGB(56, 56, 56),BorderSizePixel = 1,Size = UDim2.new(1,0,1,0),Position = UDim2.new(0.5,0,0.5,0),ZIndex = 9902,Parent = loaderOutline2})
	local loaderTitle = utility.new("TextLabel",{AnchorPoint = Vector2.new(0.5,0),BackgroundTransparency = 1,Size = UDim2.new(1,-10,0,20),Position = UDim2.new(0.5,0,0,0),Font = "RobotoMono",Text = loaderName,TextColor3 = Color3.fromRGB(168, 52, 235),TextXAlignment = "Center",TextSize = 12,TextStrokeTransparency = 0,ZIndex = 9903,Parent = loaderIndent})
	local scripttitle = utility.new("TextLabel",{AnchorPoint = Vector2.new(0.5,0),BackgroundTransparency = 1,Size = UDim2.new(1,-10,0,20),Position = UDim2.new(0.5,0,0,20),Font = "RobotoMono",Text = "Script: "..scriptname,TextColor3 = Color3.fromRGB(255, 255, 255),TextXAlignment = "Center",TextSize = 12,TextStrokeTransparency = 0,ZIndex = 9903,Parent = loaderIndent})
	local makebutton = function(bname,parent)
		...
		local button_button = utility.new("TextButton",{AnchorPoint = Vector2.new(0,0),BackgroundTransparency = 1,Size = UDim2.new(1,0,1,0),Position = UDim2.new(0,0,0,0),Text = bname,TextColor3 = Color3.fromRGB(255,255,255),TextSize = 12,TextStrokeTransparency = 0,Font = "RobotoMono",ZIndex = 9908,Parent = button_holder})
	...
	loader = {["outline"] = loaderOutline,["outline2"] = loaderOutline2,["indent"] = loaderIndent,["title"] = loaderTitle}
	setmetatable(loader,loaders)
	return loader
end

function loaders:toggle() self.outline.Visible = true end

function pages:section(props)
	local sectionName = props.name or "new ui"
	local side = props.side or "left"
	local size = props.size or 200
	side = side:lower()
	local section = {}
	...
	local sectionOutline = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(24, 24, 24),BorderColor3 = Color3.fromRGB(12, 12, 12),BorderMode = "Inset",BorderSizePixel = 1,Size = UDim2.new(1,0,1,0),Parent = sectionholder})
	local sectionColor = utility.new("Frame",{AnchorPoint = Vector2.new(0.5,0),BackgroundColor3 = self.library.theme.accent,BorderSizePixel = 0,Size = UDim2.new(1,-2,0,1),Position = UDim2.new(0.5,0,0,0),Parent = sectionOutline})
	table.insert(self.library.themeitems["accent"]["BackgroundColor3"],sectionColor)
	local content = utility.new("Frame",{AnchorPoint = Vector2.new(0.5,1),BackgroundTransparency = 1,BorderSizePixel = 0,Size = UDim2.new(1,-12,1,-25),Position = UDim2.new(0.5,0,1,-5),Parent = sectionOutline})
	local sectionTitle = utility.new("TextLabel",{BackgroundTransparency = 1,Size = UDim2.new(1,-5,0,20),Position = UDim2.new(0,5,0,0),Font = self.library.font,Text = sectionName,TextColor3 = Color3.fromRGB(255,255,255),TextSize = self.library.textsize,TextStrokeTransparency = 0,TextXAlignment = "Left",Parent = sectionOutline})
	utility.new("UIListLayout",{FillDirection = "Vertical",Padding = UDim.new(0,5),Parent = content})
	section = {["library"] = self.library,["sectionholder"] = sectionholder,["color"] = sectionColor,["content"] = content,["pointers"] = {}}
...
function pages:multisection(props)
	local msName = props.name or "new ui"
	local side = props.side or "left"
	local size = props.size or 200
	side = side:lower()
	local multisection = {}
	local sectionholder = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(24, 24, 24),BorderColor3 = Color3.fromRGB(56, 56, 56),BorderMode = "Inset",BorderSizePixel = 1,Size = UDim2.new(1,0,0,size),Parent = self[side]})
	local msOutline = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(24, 24, 24),BorderColor3 = Color3.fromRGB(12, 12, 12),BorderMode = "Inset",BorderSizePixel = 1,Size = UDim2.new(1,0,1,0),Parent = sectionholder})
	local msColor = utility.new("Frame",{AnchorPoint = Vector2.new(0.5,0),BackgroundColor3 = self.library.theme.accent,BorderSizePixel = 0,Size = UDim2.new(1,-2,0,1),Position = UDim2.new(0.5,0,0,0),Parent = msOutline})
	table.insert(self.library.themeitems["accent"]["BackgroundColor3"],msColor)
	local tabsholder = utility.new("Frame",{AnchorPoint = Vector2.new(0,1),BackgroundTransparency = 1,BorderSizePixel = 0,Size = UDim2.new(1,0,1,-15),Position = UDim2.new(0,0,1,0),Parent = msOutline})
	local msTitle = utility.new("TextLabel",{BackgroundTransparency = 1,Size = UDim2.new(1,-5,0,20),Position = UDim2.new(0,5,0,0),Font = self.library.font,Text = msName,TextColor3 = Color3.fromRGB(255,255,255),TextSize = self.library.textsize,TextStrokeTransparency = 0,TextXAlignment = "Left",Parent = msOutline})
	local msButtons = utility.new("Frame",{AnchorPoint = Vector2.new(0.5,0),BackgroundTransparency = 1,BorderSizePixel = 0,Size = UDim2.new(1,-6,0,20),Position = UDim2.new(0.5,0,0,5),Parent = tabsholder})
	local msTabs = utility.new("Frame",{AnchorPoint = Vector2.new(0.5,1),BackgroundColor3 = Color3.fromRGB(20, 20, 20),BorderColor3 = Color3.fromRGB(12, 12, 12),BorderMode = "Inset",BorderSizePixel = 1,Size = UDim2.new(1,-6,1,-27),Position = UDim2.new(0.5,0,1,-3),Parent = tabsholder})
	utility.new("UIListLayout",{FillDirection = "Horizontal",Padding = UDim.new(0,2),Parent = msButtons})
	local tabs_outline = utility.new("Frame",{AnchorPoint = Vector2.new(0,0),BackgroundColor3 = Color3.fromRGB(24, 24, 24),BorderColor3 = Color3.fromRGB(56, 56, 56),BorderMode = "Inset",BorderSizePixel = 1,Size = UDim2.new(1,0,1,0),Position = UDim2.new(0,0,0,0),Parent = msTabs})
	multisection = {["library"] = self.library,["sectionholder"] = sectionholder,["color"] = msColor,["tabsholder"] = tabsholder,["mssections"] = {},["buttons"] = msButtons,["tabs"] = msTabs,["tabs_outline"] = tabs_outline,["pointers"] = {}}
...
function multisections:section(props)
	local mssName = props.name or "new ui"
	local mssection = {}
	...
	local mssOutline = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(20, 20, 20),BorderColor3 = Color3.fromRGB(56, 56, 56),BorderMode = "Inset",BorderSizePixel = 1,Size = UDim2.new(1,0,1,0),Position = UDim2.new(0,0,0,0),Parent = tabbutton})
	local button = utility.new("TextButton",{AnchorPoint = Vector2.new(0,0),BackgroundTransparency = 1,Size = UDim2.new(1,0,1,0),Position = UDim2.new(0,0,0,0),Text = "",Parent = tabbutton})
	local r_line = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(56, 56, 56),BorderSizePixel = 0,Size = UDim2.new(0,1,0,1),Position = UDim2.new(1,0,1,1),ZIndex = 2,Parent = mssOutline})
	local l_line = utility.new("Frame",{AnchorPoint = Vector2.new(1,0),BackgroundColor3 = Color3.fromRGB(56, 56, 56),BorderSizePixel = 0,Size = UDim2.new(0,1,0,1),Position = UDim2.new(0,0,1,1),ZIndex = 2,Parent = mssOutline})
	local line = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(20, 20, 20),BorderSizePixel = 0,Size = UDim2.new(1,0,0,2),Position = UDim2.new(0,0,1,0),ZIndex = 2,Parent = mssOutline})
	local label = utility.new("TextLabel",{BackgroundTransparency = 1,Size = UDim2.new(1,0,0,20),Position = UDim2.new(0,0,0,0),Font = self.library.font,Text = mssName,TextColor3 = Color3.fromRGB(255,255,255),TextSize = self.library.textsize,TextStrokeTransparency = 0,Parent = mssOutline})
	local content = utility.new("Frame",{AnchorPoint = Vector2.new(0.5,1),BackgroundTransparency = 1,BorderSizePixel = 0,Size = UDim2.new(1,-6,1,-27),Position = UDim2.new(0.5,0,1,-3),Visible = false,Parent = self.tabs_outline})
	utility.new("UIListLayout",{FillDirection = "Vertical",Padding = UDim.new(0,5),Parent = content})
	mssection = {["library"] = self.library,["outline"] = mssOutline,["r_line"] = r_line,["l_line"] = l_line,["line"] = line,["content"] = content,["open"] = false,["pointers"] = {}}
...
function sections:toggle(props)
	...
	local trans = 1
	if def then col = self.library.theme.accent trans = 0 end
...
function sections:button(props)
	...
	buttonTbl = {["library"] = self.library,["tooltip"] = tooltipstr}
...
function sections:slider(props)
	local sliderName = props.name or "new slider"
	...
	local sliderOutline = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(24, 24, 24),BorderColor3 = Color3.fromRGB(12, 12, 12),BorderMode = "Inset",BorderSizePixel = 1,Size = UDim2.new(1,0,0,12),Position = UDim2.new(0,0,0,15),Parent = sliderholder})
	local sliderOutline2 = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(30, 30, 30),BorderColor3 = Color3.fromRGB(56, 56, 56),BorderMode = "Inset",BorderSizePixel = 1,Size = UDim2.new(1,0,1,0),Parent = sliderOutline})
	local sliderValue = utility.new("TextLabel",{BackgroundTransparency = 1,Size = UDim2.new(1,0,0,2),Position = UDim2.new(0,0,0.5,0),Font = self.library.font,Text = def..measurement.."/"..max..measurement,TextColor3 = Color3.fromRGB(255,255,255),TextSize = self.library.textsize,TextStrokeTransparency = 0,ZIndex = 3,Parent = sliderOutline})
	local sliderColorFrame = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(30, 30, 30),BorderSizePixel = 0,Size = UDim2.new(1,0,1,0),Parent = sliderOutline2})
	utility.new("UIGradient",{Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(199, 191, 204)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(255, 255, 255))},Rotation = 90,Parent = sliderColorFrame})
	local slide = utility.new("Frame",{BackgroundColor3 = self.library.theme.accent,BorderSizePixel = 0,Size = UDim2.new((1 / sliderColorFrame.AbsoluteSize.X) * (sliderColorFrame.AbsoluteSize.X / (max - min) * (def - min)),0,1,0),ZIndex = 2,Parent = sliderOutline})
	...
	local sliderTitle = utility.new("TextLabel",{BackgroundTransparency = 1,Size = UDim2.new(1,0,0,15),Position = UDim2.new(0,0,0,0),Font = self.library.font,Text = sliderName,TextColor3 = Color3.fromRGB(255,255,255),TextSize = self.library.textsize,TextStrokeTransparency = 0,TextXAlignment = "Left",Parent = sliderholder})
	slider = {["library"] = self.library,["outline"] = sliderOutline,["sliderbutton"] = sliderbutton,["title"] = sliderTitle,["value"] = sliderValue,["slide"] = slide,["color"] = sliderColorFrame,["max"] = max,["min"] = min,["current"] = def,["measurement"] = measurement,["tick"] = ticking,["rounding"] = rounding,["callback"] = callback,["tooltip"] = tooltipstr}
	local function doslide()
		...
		local newres = result
		if slider.rounding then newres = math.floor(result) else newres = utility.round(result, 2) end
	...
	sliderbutton.MouseButton1Down:Connect(function() slider.holding = true doslide() table.insert(self.library.themeitems["accent"]["BorderColor3"],sliderOutline) sliderOutline.BorderColor3 = self.library.theme.accent end)
	sliderbutton.MouseEnter:Connect(function() self.library.showTooltip(slider.tooltip, sliderbutton) end)
	sliderbutton.MouseLeave:Connect(function() self.library.hideTooltip() end)
	uis.InputChanged:Connect(function() if slider.holding then doslide() end end)
	uis.InputEnded:Connect(function(Input) if Input.UserInputType.Name == 'MouseButton1' and slider.holding then slider.holding = false sliderOutline.BorderColor3 = Color3.fromRGB(12, 12, 12) local find = table.find(self.library.themeitems["accent"]["BorderColor3"],sliderOutline) if find then table.remove(self.library.themeitems["accent"]["BorderColor3"],find) end end end)
...
function sliders:set(val)
	local newres = val
	if self.rounding then newres = math.floor(val) else newres = utility.round(val, 2) end
...
function dropdowns:set(val)
	if val ~= nil then
		if table.find(self.options,val) then
			...
			for z,x in pairs(self.titles) do if x.Text == val then x.TextColor3 = self.library.theme.accent else x.TextColor3 = Color3.fromRGB(255,255,255) end end
		end
	end
end

function sections:multibox(props)
	local mbName = props.name or "new ui"
	local def = props.def or {}
	local max = props.max or 4
	local options = props.options or {}
	local callback = props.callback or function()end
	local tooltipstr = props.tooltip or "N/A"
	local defstr = ""
	if #def > 1 then for i,v in pairs(def) do if i == #def then defstr = defstr..v else defstr = defstr..v..", " end end else for i,v in pairs(def) do defstr = defstr..v end end
	local multibox = {}
	local multiboxholder = utility.new("Frame",{BackgroundTransparency = 1,Size = UDim2.new(1,0,0,35),ZIndex = 2,Parent = self.content})
	local mbOutline = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(24, 24, 24),BorderColor3 = Color3.fromRGB(12, 12, 12),BorderMode = "Inset",BorderSizePixel = 1,Size = UDim2.new(1,0,0,20),Position = UDim2.new(0,0,0,15),Parent = multiboxholder})
	local mbOutline2 = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(24, 24, 24),BorderColor3 = Color3.fromRGB(56, 56, 56),BorderMode = "Inset",BorderSizePixel = 1,Size = UDim2.new(1,0,1,0),Parent = mbOutline})
	local mbColor = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(30, 30, 30),BorderSizePixel = 0,Size = UDim2.new(1,0,1,0),Parent = mbOutline2})
	utility.new("UIGradient",{Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(199, 191, 204)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(255, 255, 255))},Rotation = 90,Parent = mbColor})
	local mbValue = utility.new("TextLabel",{AnchorPoint = Vector2.new(0,0),BackgroundTransparency = 1,Size = UDim2.new(1,-20,1,0),Position = UDim2.new(0,5,0,0),Font = self.library.font,Text = defstr,TextColor3 = Color3.fromRGB(255,255,255),TextSize = self.library.textsize,TextStrokeTransparency = 0,TextXAlignment = "Left",ClipsDescendants = true,Parent = mbOutline})
	local indicator = utility.new("TextLabel",{AnchorPoint = Vector2.new(0.5,0),BackgroundTransparency = 1,Size = UDim2.new(1,-10,1,0),Position = UDim2.new(0.5,0,0,0),Font = self.library.font,Text = "+",TextColor3 = Color3.fromRGB(255,255,255),TextSize = self.library.textsize,TextStrokeTransparency = 0,TextXAlignment = "Right",ClipsDescendants = true,Parent = mbOutline})
	local mbTitle = utility.new("TextLabel",{BackgroundTransparency = 1,Size = UDim2.new(1,0,0,15),Position = UDim2.new(0,0,0,0),Font = self.library.font,Text = mbName,TextColor3 = Color3.fromRGB(255,255,255),TextSize = self.library.textsize,TextStrokeTransparency = 0,TextXAlignment = "Left",Parent = multiboxholder})
	...
	local optionsholder = utility.new("Frame",{BackgroundTransparency = 1,BorderColor3 = Color3.fromRGB(56, 56, 56),BorderMode = "Inset",BorderSizePixel = 1,Size = UDim2.new(1,0,0,0),Position = UDim2.new(0,0,0,34),Visible = false,ClipsDescendants = true,Parent = multiboxholder})
	local mbSize = math.clamp(#options,1,max)
	local optionsoutline = utility.new("ScrollingFrame",{BackgroundColor3 = Color3.fromRGB(56, 56, 56),BorderColor3 = Color3.fromRGB(56, 56, 56),BorderMode = "Inset",BorderSizePixel = 1,Size = UDim2.new(1,0,mbSize,2),Position = UDim2.new(0,0,0,0),ClipsDescendants = true,CanvasSize = UDim2.new(0,0,0,18*#options),ScrollBarImageTransparency = 0.25,ScrollBarImageColor3 = Color3.fromRGB(0,0,0),ScrollBarThickness = 5,VerticalScrollBarInset = "ScrollBar",VerticalScrollBarPosition = "Right",ZIndex = 5,Parent = optionsholder})
	utility.new("UIListLayout",{FillDirection = "Vertical",Parent = optionsoutline})
	multibox = {["library"] = self.library,["indicator"] = indicator,["optionsholder"] = optionsholder,["options"] = options,["value"] = mbValue,["open"] = false,["titles"] = {},["current"] = def,["callback"] = callback,["tooltip"] = tooltipstr}
	table.insert(multibox.library.multiboxes,multibox)
	for i,v in pairs(options) do
		local mboptionbutton = utility.new("TextButton",{AnchorPoint = Vector2.new(0,0),BackgroundTransparency = 1,Size = UDim2.new(1,0,0,18),Text = "",ZIndex = 6,Parent = optionsoutline})
		local mboptiontitle = utility.new("TextLabel",{AnchorPoint = Vector2.new(0.5,0),BackgroundTransparency = 1,Size = UDim2.new(1,-10,1,0),Position = UDim2.new(0.5,0,0,0),Font = self.library.font,Text = v,TextColor3 = Color3.fromRGB(255,255,255),TextSize = self.library.textsize,TextStrokeTransparency = 0,TextXAlignment = "Left",ClipsDescendants = true,ZIndex = 6,Parent = mboptionbutton})
		...
		mboptionbutton.MouseButton1Down:Connect(function()
			...
			if find == nil then
				...
				local str = ""
				if #multibox.current > 1 then for ii,vv in pairs(multibox.current) do if ii == #multibox.current then str = str..vv else str = str..vv..", " end end else for ii,vv in pairs(multibox.current) do str = str..vv end end
				mbValue.Text = str
	...
	dropdownbutton.MouseButton1Down:Connect(function()
		...
		for i,v in pairs(multibox.titles) do if v.TextColor3 ~= Color3.fromRGB(255,255,255) then v.TextColor3 = self.library.theme.accent end end
		if not multibox.open then
			optionsholder.Visible = true
			ts:Create(optionsholder, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(1,0,0,18*mbSize + 2)}):Play()
			multibox.open = true
			indicator.Text = "-"
		else
			local t = ts:Create(optionsholder, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(1,0,0,0)})
			t:Play()
			t.Completed:Connect(function() if not multibox.open then optionsholder.Visible = false end end)
			multibox.open = false
			indicator.Text = "+"
		end
	end)
	dropdownbutton.MouseEnter:Connect(function() self.library.showTooltip(multibox.tooltip, dropdownbutton) end)
	dropdownbutton.MouseLeave:Connect(function() self.library.hideTooltip() end)
	if props.pointer then self.library.pointers[tostring(props.pointer)] = multibox end
...
function sections:textbox(props)
	local tbName = props.name or "new ui"
	local def = props.def or ""
	...
	local tbOutline = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(24, 24, 24),BorderColor3 = Color3.fromRGB(12, 12, 12),BorderMode = "Inset",BorderSizePixel = 1,Size = UDim2.new(1,0,0,20),Position = UDim2.new(0,0,0,15),Parent = textboxholder})
	local tbOutline2 = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(24, 24, 24),BorderColor3 = Color3.fromRGB(56, 56, 56),BorderMode = "Inset",BorderSizePixel = 1,Size = UDim2.new(1,0,1,0),Parent = tbOutline})
	local tbColor = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(30, 30, 30),BorderSizePixel = 0,Size = UDim2.new(1,0,1,0),Parent = tbOutline2})
	utility.new("UIGradient",{Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(199, 191, 204)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(255, 255, 255))},Rotation = 90,Parent = tbColor})
	local button = utility.new("TextButton",{AnchorPoint = Vector2.new(0,0),BackgroundTransparency = 1,Size = UDim2.new(1,0,1,0),Position = UDim2.new(0,0,0,0),Text = "",TextColor3 = Color3.fromRGB(255,255,255),TextSize = self.library.textsize,TextStrokeTransparency = 0,Font = self.library.font,Parent = textboxholder})
	local tbTitle = utility.new("TextLabel",{BackgroundTransparency = 1,Size = UDim2.new(1,0,0,15),Position = UDim2.new(0,0,0,0),Font = self.library.font,Text = tbName,TextColor3 = Color3.fromRGB(255,255,255),TextSize = self.library.textsize,TextStrokeTransparency = 0,TextXAlignment = "Left",Parent = textboxholder})
	...
	textbox = {["library"] = self.library,["tbox"] = tbox,["current"] = def,["callback"] = callback,["tooltip"] = tooltipstr}
	button.MouseButton1Down:Connect(function() tbox:CaptureFocus() end)
	button.MouseEnter:Connect(function() self.library.showTooltip(textbox.tooltip, button) end)
	button.MouseLeave:Connect(function() self.library.hideTooltip() end)
	tbox.Focused:Connect(function() tbOutline.BorderColor3 = self.library.theme.accent table.insert(self.library.themeitems["accent"]["BorderColor3"],tbOutline) end)
	tbox.FocusLost:Connect(function(enterPressed) textbox.current = tbox.Text callback(tbox.Text) tbOutline.BorderColor3 = Color3.fromRGB(12, 12, 12) local find = table.find(self.library.themeitems["accent"]["BorderColor3"],tbOutline) if find then table.remove(self.library.themeitems["accent"]["BorderColor3"],find) end end)
	if props.pointer then self.library.pointers[tostring(props.pointer)] = textbox end
...
function sections:keybind(props)
	local kbName = props.name or "new ui"
	local def = props.def or nil
	local callback = props.callback or function()end
	local allowed = props.allowed or 1
	local tooltipstr = props.tooltip or "N/A"
	local default = ".."
	local typeis = nil
	if typeof(def) == "EnumItem" then
		if def == Enum.UserInputType.MouseButton1 then if allowed == 1 then default = "MB1" typeis = "UserInputType" end
		elseif def == Enum.UserInputType.MouseButton2 then if allowed == 1 then default = "MB2" typeis = "UserInputType" end
		elseif def == Enum.UserInputType.MouseButton3 then if allowed == 1 then default = "MB3" typeis = "UserInputType" end
		else
	...
	local kbOutline = utility.new("Frame",{AnchorPoint = Vector2.new(1,0),BackgroundColor3 = Color3.fromRGB(24, 24, 24),BorderColor3 = Color3.fromRGB(12, 12, 12),BorderMode = "Inset",BorderSizePixel = 1,Size = UDim2.new(0,40,1,0),Position = UDim2.new(1,0,0,0),Parent = keybindholder})
	local kbOutline2 = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(24, 24, 24),BorderColor3 = Color3.fromRGB(56, 56, 56),BorderMode = "Inset",BorderSizePixel = 1,Size = UDim2.new(1,0,1,0),Position = UDim2.new(0,0,0,0),Parent = kbOutline})
	local kbValue = utility.new("TextLabel",{BackgroundTransparency = 1,Size = UDim2.new(1,0,1,0),Position = UDim2.new(0,0,0,0),Font = self.library.font,Text = default,TextColor3 = Color3.fromRGB(255,255,255),TextSize = self.library.textsize,TextStrokeTransparency = 0,TextXAlignment = "Center",Parent = kbOutline})
	kbOutline.Size = UDim2.new(0,kbValue.TextBounds.X+20,1,0)
	local kbColor = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(30, 30, 30),BorderSizePixel = 0,Size = UDim2.new(1,0,1,0),Position = UDim2.new(0,0,0,0),Parent = kbOutline2})
	utility.new("UIGradient",{Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(199, 191, 204)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(255, 255, 255))},Rotation = 90,Parent = kbColor})
	local button = utility.new("TextButton",{AnchorPoint = Vector2.new(0,0),BackgroundTransparency = 1,Size = UDim2.new(1,0,1,0),Position = UDim2.new(0,0,0,0),Text = "",TextColor3 = Color3.fromRGB(255,255,255),TextSize = self.library.textsize,TextStrokeTransparency = 0,Font = self.library.font,Parent = keybindholder})
	local kbTitle = utility.new("TextLabel",{BackgroundTransparency = 1,Size = UDim2.new(1,0,1,0),Position = UDim2.new(0,0,0,0),Font = self.library.font,Text = kbName,TextColor3 = Color3.fromRGB(255,255,255),TextSize = self.library.textsize,TextStrokeTransparency = 0,TextXAlignment = "Left",Parent = keybindholder})
	keybind = {["library"] = self.library,["down"] = false,["outline"] = kbOutline,["value"] = kbValue,["allowed"] = allowed,["current"] = {typeis,utility.splitenum(def)},["pressed"] = false,["callback"] = callback,["tooltip"] = tooltipstr}
	button.MouseButton1Down:Connect(function() if keybind.down == false then kbOutline.BorderColor3 = self.library.theme.accent table.insert(self.library.themeitems["accent"]["BorderColor3"],kbOutline) wait() keybind.down = true end end)
	button.MouseButton2Down:Connect(function() keybind.down = false keybind.current = {nil,nil} kbOutline.BorderColor3 = Color3.fromRGB(12, 12, 12) local find = table.find(self.library.themeitems["accent"]["BorderColor3"],kbOutline) if find then table.remove(self.library.themeitems["accent"]["BorderColor3"],find) end kbValue.Text = ".." kbOutline.Size = UDim2.new(0,kbValue.TextBounds.X+20,1,0) end)
	button.MouseEnter:Connect(function() self.library.showTooltip(keybind.tooltip, button) end)
	button.MouseLeave:Connect(function() self.library.hideTooltip() end)
	...
	uis.InputBegan:Connect(function(Input)
		if keybind.down then
			if Input.UserInputType == Enum.UserInputType.Keyboard then
				...
				if #capd > 1 then kbValue.Text = capd else kbValue.Text = Input.KeyCode.Name end
				turn("KeyCode",Input.KeyCode)
				callback(Input.KeyCode)
			end
			if allowed == 1 then
				if Input.UserInputType == Enum.UserInputType.MouseButton1 then kbValue.Text = "MB1" turn("UserInputType",Input) callback(Input)
				elseif Input.UserInputType == Enum.UserInputType.MouseButton2 then kbValue.Text = "MB2" turn("UserInputType",Input) callback(Input)
				elseif Input.UserInputType == Enum.UserInputType.MouseButton3 then kbValue.Text = "MB3" turn("UserInputType",Input) callback(Input) end
			end
		end
	end)
	if props.pointer then self.library.pointers[tostring(props.pointer)] = keybind end
...
function keybinds:set(key)
	if key then
		if typeof(key) == "table" then if key[1] and key[2] then local s, e = pcall(function() key = Enum[key[1]][key[2]] end) if not s then return end else return end end
		if typeof(key) == "EnumItem" then
			local typeis = ""
			local default = ".."
			if key == Enum.UserInputType.MouseButton1 then if self.allowed == 1 then default = "MB1" typeis = "UserInputType" end
			elseif key == Enum.UserInputType.MouseButton2 then if self.allowed == 1 then default = "MB2" typeis = "UserInputType" end
			elseif key == Enum.UserInputType.MouseButton3 then if self.allowed == 1 then default = "MB3" typeis = "UserInputType" end
			else local capd = utility.capatalize(key.Name) if #capd > 1 then default = capd else default = key.Name end typeis = "KeyCode" end
			self.value.Text = default
			self.outline.Size = UDim2.new(0, self.value.TextBounds.X+20, 1, 0)
			self.current = {typeis, utility.splitenum(key)}
			self.callback(key)
			if self.down then self.down = false self.outline.BorderColor3 = Color3.fromRGB(12, 12, 12) local find = table.find(self.library.themeitems["accent"]["BorderColor3"], self.outline) if find then table.remove(self.library.themeitems["accent"]["BorderColor3"], find) end end
		end
	end
end

function sections:colorpicker(props)
	local cpName = props.name or "new colorpicker"
	local def = props.def or Color3.fromRGB(255,255,255)
	local callback = props.callback or function()end
	local tooltipstr = props.tooltip or "N/A"
	...
	local cpOutline = utility.new("Frame",{AnchorPoint = Vector2.new(1,0),BackgroundColor3 = Color3.fromRGB(24, 24, 24),BorderColor3 = Color3.fromRGB(12, 12, 12),BorderMode = "Inset",BorderSizePixel = 1,Size = UDim2.new(0,30,1,0),Position = UDim2.new(1,0,0,0),Parent = colorpickerholder})
	local cpOutline2 = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(24, 24, 24),BorderColor3 = Color3.fromRGB(56, 56, 56),BorderMode = "Inset",BorderSizePixel = 1,Size = UDim2.new(1,0,1,0),Parent = cpOutline})
	local cpcolor = utility.new("Frame",{BackgroundColor3 = def,BorderSizePixel = 0,Size = UDim2.new(1,0,1,0),Parent = cpOutline2})
	...
	local cpTitle = utility.new("TextLabel",{BackgroundTransparency = 1,Size = UDim2.new(1,0,1,0),Position = UDim2.new(0,0,0,0),Font = self.library.font,Text = cpName,TextColor3 = Color3.fromRGB(255,255,255),TextSize = self.library.textsize,TextStrokeTransparency = 0,TextXAlignment = "Left",Parent = colorpickerholder})
	local button = utility.new("TextButton",{AnchorPoint = Vector2.new(0,0),BackgroundTransparency = 1,Size = UDim2.new(1,0,1,0),Position = UDim2.new(0,0,0,0),Text = "",TextColor3 = Color3.fromRGB(255,255,255),TextSize = self.library.textsize,TextStrokeTransparency = 0,Font = self.library.font,Parent = colorpickerholder})
	...
	local cpholderOutline2 = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(24, 24, 24),BorderColor3 = Color3.fromRGB(56, 56, 56),BorderMode = "Inset",BorderSizePixel = 1,Size = UDim2.new(1,0,1,0),ZIndex = 5,Parent = cpholder})
	local cpAccentLine = utility.new("Frame",{AnchorPoint = Vector2.new(0.5,0),BackgroundColor3 = self.library.theme.accent,BorderSizePixel = 0,Size = UDim2.new(1,-2,0,1),Position = UDim2.new(0.5,0,0,0),ZIndex = 5,Parent = cpholderOutline2})
	table.insert(self.library.themeitems["accent"]["BackgroundColor3"],cpAccentLine)
	local cpholderTitle = utility.new("TextLabel",{AnchorPoint = Vector2.new(0.5,0),BackgroundTransparency = 1,Size = UDim2.new(1,-10,0,20),Position = UDim2.new(0.5,0,0,0),Font = self.library.font,Text = cpName,TextColor3 = Color3.fromRGB(255,255,255),TextSize = self.library.textsize,TextStrokeTransparency = 0,TextXAlignment = "Left",ZIndex = 5,Parent = cpholderOutline2})
	local cpholder2 = utility.new("Frame",{AnchorPoint = Vector2.new(0,0),BackgroundColor3 = Color3.fromRGB(24, 24, 24),BorderColor3 = Color3.fromRGB(12, 12, 12),BorderMode = "Inset",BorderSizePixel = 1,Size = UDim2.new(0.875,0,0,150),Position = UDim2.new(0,5,0,20),ZIndex = 5,Parent = cpholderOutline2})
	local cpPickerOutline = utility.new("Frame",{BackgroundColor3 = Color3.fromHSV(h,1,1),BorderColor3 = Color3.fromRGB(56, 56, 56),BorderMode = "Inset",BorderSizePixel = 1,Size = UDim2.new(1,0,1,0),ZIndex = 5,Parent = cpholder2})
	local cpimage = utility.new("ImageButton",{AutoButtonColor = false,BackgroundTransparency = 1,BorderSizePixel = 0,Size = UDim2.new(1,0,1,0),ZIndex = 5,Image = "rbxassetid://7074305282",Parent = cpPickerOutline})
	local cpcursor = utility.new("ImageLabel",{AnchorPoint = Vector2.new(0.5,0.5),BackgroundTransparency = 1,BorderSizePixel = 0,Size = UDim2.new(0,6,0,6),Position = UDim2.new(s,0,1-v,0),ZIndex = 5,Image = "rbxassetid://7074391319",Parent = cpimage})
	local huepicker = utility.new("Frame",{AnchorPoint = Vector2.new(1,0),BackgroundColor3 = Color3.fromRGB(24, 24, 24),BorderColor3 = Color3.fromRGB(12, 12, 12),BorderMode = "Inset",BorderSizePixel = 1,Size = UDim2.new(0.05,0,0,150),Position = UDim2.new(1,-5,0,20),ZIndex = 5,Parent = cpholderOutline2})
	local hueOutline = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(255, 255, 255),BorderColor3 = Color3.fromRGB(56, 56, 56),BorderMode = "Inset",BorderSizePixel = 1,Size = UDim2.new(1,0,1,0),ZIndex = 5,Parent = huepicker})
	...
	local function maketextbox(parent,tbsize,tbposition)
		local textbox_holder = utility.new("Frame",{BackgroundTransparency = 1,BorderSizePixel = 0,Position = tbposition,Size = tbsize,ZIndex = 5,Parent = parent})
		local tbOutline5 = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(24, 24, 24),BorderColor3 = Color3.fromRGB(12, 12, 12),BorderMode = "Inset",BorderSizePixel = 1,Position = UDim2.new(0,0,0,0),Size = UDim2.new(1,0,1,0),ZIndex = 5,Parent = textbox_holder})
		local tbOutline6 = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(24, 24, 24),BorderColor3 = Color3.fromRGB(56, 56, 56),BorderMode = "Inset",BorderSizePixel = 1,Position = UDim2.new(0,0,0,0),Size = UDim2.new(1,0,1,0),ZIndex = 5,Parent = tbOutline5})
		local tbColor2 = utility.new("Frame",{AnchorPoint = Vector2.new(0,0),BackgroundColor3 = Color3.fromRGB(30, 30, 30),BorderSizePixel = 0,Size = UDim2.new(1,0,0,0),Position = UDim2.new(0,0,0,0),ZIndex = 5,Parent = tbOutline6})
		utility.new("UIGradient",{Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(199, 191, 204)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(255, 255, 255))},Rotation = 90,Parent = tbColor2})
		local tbox2 = utility.new("TextBox",{AnchorPoint = Vector2.new(0.5,0),BackgroundTransparency = 1,Size = UDim2.new(1,0,1,0),Position = UDim2.new(0.5,0,0,0),PlaceholderColor3 = Color3.fromRGB(255,255,255),PlaceholderText = "",Text = "",TextColor3 = Color3.fromRGB(255,255,255),TextSize = self.library.textsize,TextStrokeTransparency = 0,Font = self.library.font,ZIndex = 5,Parent = textbox_holder})
		...
		tbox_button.MouseButton1Down:Connect(function() tbox2:CaptureFocus() end)
		return {textbox_holder,tbox2,tbOutline5}
	end
	
	local red = maketextbox(cpholderOutline2,UDim2.new(0,62,0,20),UDim2.new(0,5,0,175))
	local green = maketextbox(cpholderOutline2,UDim2.new(0,62,0,20),UDim2.new(0,5,0,175)) green[1].AnchorPoint = Vector2.new(0.5,0) green[1].Position = UDim2.new(0.5,0,0,175)
	local blue = maketextbox(cpholderOutline2,UDim2.new(0,62,0,20),UDim2.new(0,5,0,175)) blue[1].AnchorPoint = Vector2.new(1,0) blue[1].Position = UDim2.new(1,-5,0,175)
	local hex = maketextbox(cpholderOutline2,UDim2.new(1,-10,0,20),UDim2.new(0,5,0,200)) hex[2].Size = UDim2.new(1,-12,1,0) hex[2].TextXAlignment = "Left"
	
	colorpicker = {["library"] = self.library,["cpholder"] = cpholder,["cpcolor"] = cpcolor,["huecursor"] = huecursor,["outline3"] = cpPickerOutline,["huecursor_inline"] = huecursor_inline,["cpcursor"] = cpcursor,["current"] = def,["open"] = false,["cp"] = false,["hue"] = false,["hsv"] = {h,s,v},["red"] = red[2],["green"] = green[2],["blue"] = blue[2],["hex"] = hex[2],["callback"] = callback,["tooltip"] = tooltipstr}
	...
	button.MouseEnter:Connect(function() self.library.showTooltip(colorpicker.tooltip, button) end)
	button.MouseLeave:Connect(function() self.library.hideTooltip() end)
	huebutton.MouseButton1Down:Connect(function() colorpicker.hue = true movehue() end)
	cpimage.MouseButton1Down:Connect(function() colorpicker.cp = true movecp() end)
	uis.InputChanged:Connect(function() if colorpicker.cp then movecp() end if colorpicker.hue then movehue() end end)
	uis.InputEnded:Connect(function(Input) if Input.UserInputType.Name == 'MouseButton1' then if colorpicker.cp then colorpicker.cp = false end if colorpicker.hue then colorpicker.hue = false end end end)
	...
	red[2].FocusLost:Connect(function()
		...
		if num then
			saved = tostring(math.clamp(tonumber(saved),0,255)) red[2].Text = ""
			if saved then if #saved >= 1 and #saved <= 3 then red[2].PlaceholderText = "R: "..tostring(saved) end colorpicker:set(Color3.fromRGB(tonumber(saved),colorpicker.current.G*255,colorpicker.current.B*255)) red[3].BorderColor3 = Color3.fromRGB(12,12,12) else red[3].BorderColor3 = Color3.fromRGB(12,12,12) end
		else red[2].Text = "" red[3].BorderColor3 = Color3.fromRGB(12,12,12) end
	end)
	...
	green[2].FocusLost:Connect(function()
		...
		local num = tonumber(saved)
		if num then
			saved = tostring(math.clamp(tonumber(saved),0,255)) green[2].Text = ""
			if saved then if #saved >= 1 and #saved <= 3 then green[2].PlaceholderText = "G: "..tostring(saved) end colorpicker:set(Color3.fromRGB(colorpicker.current.R*255,tonumber(saved),colorpicker.current.B*255)) green[3].BorderColor3 = Color3.fromRGB(12,12,12) else green[3].BorderColor3 = Color3.fromRGB(12,12,12) end
		else green[2].Text = "" green[3].BorderColor3 = Color3.fromRGB(12,12,12) end
	end)
	...
	blue[2].FocusLost:Connect(function()
		...
		local num = tonumber(saved)
		if num then
			saved = tostring(math.clamp(tonumber(saved),0,255)) blue[2].Text = ""
			if saved then if #saved >= 1 and #saved <= 3 then blue[2].PlaceholderText = "B: "..tostring(saved) end colorpicker:set(Color3.fromRGB(colorpicker.current.R*255,colorpicker.current.G*255,tonumber(saved))) blue[3].BorderColor3 = Color3.fromRGB(12,12,12) else blue[3].BorderColor3 = Color3.fromRGB(12,12,12) end
		else blue[2].Text = "" blue[3].BorderColor3 = Color3.fromRGB(12,12,12) end
	end)
	...
	hex[2].FocusLost:Connect(function()
		...
		if #saved >= 6 and #saved <= 7 then
			local e,s2 = pcall(function() utility.from_hex(saved) end)
			if e == true then local hexcolor = utility.from_hex(saved) if hexcolor then colorpicker:set(hexcolor) hex[2].Text = "" hex[3].BorderColor3 = Color3.fromRGB(12,12,12) else hex[2].Text = "" hex[3].BorderColor3 = Color3.fromRGB(12,12,12) end else hex[2].Text = "" hex[3].BorderColor3 = Color3.fromRGB(12,12,12) end
		else hex[2].Text = "" hex[3].BorderColor3 = Color3.fromRGB(12,12,12) end
	end)
	if props.pointer then self.library.pointers[tostring(props.pointer)] = colorpicker end
...
function colorpickers:set(newColor)
	if newColor then
		if typeof(newColor) == "table" then newColor = Color3.fromRGB(newColor[1]*255,newColor[2]*255,newColor[3]*255) end
		local h,s,v = newColor:ToHSV()
		self.outline3.BackgroundColor3 = Color3.fromHSV(h,1,1)
		self.huecursor_inline.BackgroundColor3 = Color3.fromHSV(h,1,1)
		self.hsv[1] = h
		self.hsv[2] = s
		self.hsv[3] = v
		self.current = newColor
		self.cpcolor.BackgroundColor3 = newColor
		self.huecursor:TweenPosition(UDim2.new(0.5,0,h,0),Enum.EasingDirection.Out,Enum.EasingStyle.Quad,0.15,true)
		self.cpcursor:TweenPosition(UDim2.new(s,0,1-v,0),Enum.EasingDirection.Out,Enum.EasingStyle.Quad,0.15,true)
		self.callback(self.current)
	end
end

function sections:buttonbox(props)
	local bbName = props.name or "new ui"
	local def = props.def or ""
	local max = props.max or 4
	local options = props.options or {}
	local callback = props.callback or function()end
	...
	local bbOutline = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(24, 24, 24),BorderColor3 = Color3.fromRGB(12, 12, 12),BorderMode = "Inset",BorderSizePixel = 1,Size = UDim2.new(1,0,0,20),Position = UDim2.new(0,0,0,15),Parent = buttonboxholder})
	local bbOutline2 = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(24, 24, 24),BorderColor3 = Color3.fromRGB(56, 56, 56),BorderMode = "Inset",BorderSizePixel = 1,Size = UDim2.new(1,0,1,0),Position = UDim2.new(0,0,0,0),Parent = bbOutline})
	local bbColor = utility.new("Frame",{BackgroundColor3 = Color3.fromRGB(30, 30, 30),BorderSizePixel = 0,Size = UDim2.new(1,0,1,0),Position = UDim2.new(0,0,0,0),Parent = bbOutline2})
	utility.new("UIGradient",{Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(199, 191, 204)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(255, 255, 255))},Rotation = 90,Parent = bbColor})
	...
	utility.new("UIListLayout",{FillDirection = "Vertical",Parent = optionsoutline})
	buttonbox = {["library"] = self.library,["optionsholder"] = optionsholder,["indicator"] = indicator,["options"] = options,["title"] = bbTitle,["open"] = false,["titles"] = {},["current"] = def,["callback"] = callback,["tooltip"] = tooltipstr}
	...
	for i,v in pairs(options) do
		...
		bboptionbutton.MouseButton1Down:Connect(function()
			local t = ts:Create(optionsholder, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(1,0,0,0)})
			t:Play()
			t.Completed:Connect(function() if not buttonbox.open then optionsholder.Visible = false end end)
	...
	buttonboxbutton.MouseButton1Down:Connect(function()
		...
		if not buttonbox.open then
			optionsholder.Visible = true
			ts:Create(optionsholder, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(1,0,0,18*bbSize + 2)}):Play()
			buttonbox.open = true
			indicator.Text = "-"
		else
			local t = ts:Create(optionsholder, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(1,0,0,0)})
	...
	buttonboxbutton.MouseEnter:Connect(function() self.library.showTooltip(buttonbox.tooltip, buttonboxbutton) end)
	buttonboxbutton.MouseLeave:Connect(function() self.library.hideTooltip() end)
	if props.pointer then self.library.pointers[tostring(props.pointer)] = buttonbox end
