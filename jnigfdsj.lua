...
--
-- // CURSOR (Drawing lines - треугольник)
local cursorLine1 = Drawing.new("Line")
cursorLine1.Visible = false
cursorLine1.Color = color
...
cursorLine2.Color = color
cursorLine2.Thickness = 1
cursorLine2.Transparency = 0

local cursorLine3 = Drawing.new("Line")
cursorLine3.Visible = false
cursorLine3.Color = color
cursorLine3.Thickness = 1
cursorLine3.Transparency = 0
...
window = {
	...
	["outline"] = outline,
	["shadow"] = shadow,
	["cursorLine1"] = cursorLine1,
	["cursorLine2"] = cursorLine2,
	["cursorLine3"] = cursorLine3,
		...
		while tooltipCurrentElement == element do
			if tick() - tooltipHoverStart >= tooltipDelay then
				if tooltipCurrentElement == element then
					...
					local width = math.min(tooltipMaxWidth, textBounds.X + 16)
					local height = textBounds.Y + 10
					tooltipFrame.Size = UDim2.new(0, width, 0, height)
					
					currentTooltipTween = ts:Create(tooltipFrame, TweenInfo.new(0.7, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 0})
					currentTooltipTween:Play()
					ts:Create(tooltipText, TweenInfo.new(0.7, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextTransparency = 0}):Play()
					ts:Create(tooltipAccent, TweenInfo.new(0.7, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 0}):Play()
				end
				break
			end
			task.wait(0.05)
...
local frameCount = 0
local fpsCheckTime = 0
local currentFPS = 60
--
rs.RenderStepped:Connect(function(deltaTime)
	frameCount = frameCount + 1
	fpsCheckTime = fpsCheckTime + deltaTime
	
	if fpsCheckTime >= 1 then
		currentFPS = math.min(60, math.max(30, frameCount))
		frameCount = 0
		fpsCheckTime = 0
	end
	
	local targetInterval = 1 / math.min(60, currentFPS)
	lastUpdateTime = lastUpdateTime + deltaTime
	
	if lastUpdateTime >= targetInterval then
		lastUpdateTime = 0
		
		if window.windowVisible and screen.Enabled then
			local mousePos = uis:GetMouseLocation()
			local mx, my = mousePos.X, mousePos.Y - 36
			
			-- Треугольник курсора (чуть повернут влево, узкий)
			local p1 = Vector2.new(mx, my)
			local p2 = Vector2.new(mx + 4, my + 18)
			local p3 = Vector2.new(mx - 2, my + 14)
			
			cursorLine1.From = p1
			cursorLine1.To = p2
			cursorLine1.Color = window.theme.accent
			cursorLine1.Visible = true
			
			cursorLine2.From = p2
			cursorLine2.To = p3
			cursorLine2.Color = window.theme.accent
			cursorLine2.Visible = true
			
			cursorLine3.From = p3
			cursorLine3.To = p1
			cursorLine3.Color = window.theme.accent
			cursorLine3.Visible = true
			
			-- Заполнение треугольника
			for i, line in ipairs(cursorFill) do
				local t = i / #cursorFill
				local startPoint = p1:Lerp(p3, t)
				local endPoint = p1:Lerp(p2, t)
				line.From = startPoint
				line.To = endPoint
				line.Color = window.theme.accent
				line.Visible = true
			end
			
			-- Tooltip position
			if tooltipVisible then
				tooltipFrame.Position = UDim2.new(0, mx + 15, 0, my)
			...
			cursorLine2.Visible = false
			cursorLine3.Visible = false
			for _, line in ipairs(cursorFill) do
				line.Visible = false
			end
		end
	end
		...
		if Input.KeyCode == window.key then
			if cooldown == false then
				if toggled then
					...
					-- Анимация закрытия: вниз + прозрачность
					local targetPos = UDim2.new(saved.X.Scale, saved.X.Offset, saved.Y.Scale, saved.Y.Offset + 50)
					
					ts:Create(outline, TweenInfo.new(1, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {Position = targetPos, BackgroundTransparency = 1}):Play()
					ts:Create(shadow, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {BackgroundTransparency = 1}):Play()
					
					for _, desc in ipairs(outline:GetDescendants()) do
						if desc:IsA("Frame") then
							ts:Create(desc, TweenInfo.new(1, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {BackgroundTransparency = 1}):Play()
						elseif desc:IsA("TextLabel") or desc:IsA("TextButton") or desc:IsA("TextBox") then
							ts:Create(desc, TweenInfo.new(1, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {TextTransparency = 1, TextStrokeTransparency = 1}):Play()
						elseif desc:IsA("ImageLabel") or desc:IsA("ImageButton") then
							ts:Create(desc, TweenInfo.new(1, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {ImageTransparency = 1}):Play()
						elseif desc:IsA("ScrollingFrame") then
							ts:Create(desc, TweenInfo.new(1, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {BackgroundTransparency = 1, ScrollBarImageTransparency = 1}):Play()
						end
					end
					...
					-- Начальная позиция (ниже) и прозрачность
					...
					-- Анимация открытия: вверх + непрозрачность
					ts:Create(outline, TweenInfo.new(1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = saved, BackgroundTransparency = 0}):Play()
					...
					for _, desc in ipairs(outline:GetDescendants()) do
						if desc:IsA("Frame") then
							...
							ts:Create(desc, TweenInfo.new(1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = origTrans}):Play()
						elseif desc:IsA("TextLabel") or desc:IsA("TextButton") or desc:IsA("TextBox") then
							ts:Create(desc, TweenInfo.new(1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextTransparency = 0, TextStrokeTransparency = 0}):Play()
						elseif desc:IsA("ImageLabel") or desc:IsA("ImageButton") then
							ts:Create(desc, TweenInfo.new(1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageTransparency = 0}):Play()
						elseif desc:IsA("ScrollingFrame") then
							local origTrans = desc:GetAttribute("OriginalTransparency") or 0
							ts:Create(desc, TweenInfo.new(1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = origTrans, ScrollBarImageTransparency = 0.25}):Play()
						end
					end
